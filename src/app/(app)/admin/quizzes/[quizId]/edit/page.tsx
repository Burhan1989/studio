...
import { getQuizById, updateQuiz, getClasses } from "@/lib/mockData";
...
const editAdminQuizSchema = z.object({
...
type EditAdminQuizFormData = z.infer<typeof editAdminQuizSchema>;
...
export default function AdminEditQuizPage() {
  ...
  const form = useForm<EditAdminQuizFormData>({
    resolver: zodResolver(editAdminQuizSchema),
    defaultValues: {
      title: "",
      description: "",
      questions: [],
  ...
  async function onSubmit(values: EditAdminQuizFormData) {
    ...
    const updatedQuizData: Quiz = {
      ...
      teacherId: initialQuizData.teacherId || user.id, // Preserve original teacherId or assign current admin
      questions: values.questions.map(q => ({
        ...q,
        id: q.id || `new_q_${Date.now()}${Math.random().toString(36).substring(2,7)}`,
    ...
    if (success) {
      toast({
        title: "Kuis Diperbarui oleh Admin",
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-3xl font-bold">Edit Kuis (Admin): {initialQuizData?.title}</h1>
        </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="cth. Kuis Pengetahuan Umum Global" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
