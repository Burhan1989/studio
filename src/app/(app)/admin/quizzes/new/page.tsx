...
import { addQuiz, getClasses } from "@/lib/mockData";
...
const newAdminQuizSchema = z.object({
...
type NewAdminQuizFormData = z.infer<typeof newAdminQuizSchema>;
...
export default function AdminNewQuizPage() {
  ...
  const form = useForm<NewAdminQuizFormData>({
    resolver: zodResolver(newAdminQuizSchema),
    defaultValues: {
      title: "",
      description: "",
  ...
  async function onSubmit(values: NewAdminQuizFormData) {
    if (!user || !user.isAdmin) {
        toast({ title: "Akses Ditolak", description: "Anda harus login sebagai admin.", variant: "destructive" });
        return;
    }
    setIsLoading(true);
    ...
    const quizDataForMock: Omit<Quiz, 'id'> & { teacherId?: string } = {
        ...
        teacherId: user.id, // Menggunakan ID admin yang membuat
    ...
    console.log("Data kuis baru oleh admin (simulasi):", quizDataForMock);
    await new Promise(resolve => setTimeout(resolve, 1000)); 
    
    addQuiz(quizDataForMock); 

    toast({
      title: "Kuis Baru Ditambahkan oleh Admin",
  ...
  const addOption = (questionIndex: number) => {
    ...
    if (currentOptions.length < 6) { 
        update(questionIndex, { ...fields[questionIndex], options: [...currentOptions, ""] });
    } else {
